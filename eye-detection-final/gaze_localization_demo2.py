...
class RandomClickCalibrator:
    ...
    - dla zaakceptowanych klatek liczone są cechy i uśredniane (1 wektor cech na punkt),
    - na końcu trenowane są dwa modele Ridge (dla X i Y).
    """
    ...
    def run(self):
        ...
        print(f"Liczba użytych punktów kalibracyjnych: {len(X)}")
...
class RealTimeGazeTrackerWithValidation:
    """
    - używa wytrenowanych modeli Ridge,
...
def main():
    ...
    calibrator = RandomClickCalibrator(
        detector, cap, feature_extractor, validator, screen_size=screen_size, alpha=alpha
    )
    ...
    tracker = RealTimeGazeTrackerWithValidation(
        ...
        model_x, model_y, screen_size=screen_size
    )
    tracker.run()
