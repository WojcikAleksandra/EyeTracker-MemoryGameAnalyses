...
from PyQt5.QtGui import QIcon, QPainter, QPen, QCursor, QPixmap, QColor, QBrush
...
class GazeDotOverlay(QWidget):
    def __init__(self, parent: QWidget, radius: int = 16):
    ...
    def set_gaze_pos(self, x: int, y: int):
    ...
    def paintEvent(self, event):
        ...
        color = QColor(0x85, 0x49, 0xC9, 150)
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(color))
        painter.drawEllipse(self._pos, self.radius, self.radius)
...
class MemoryGameWindow(QMainWindow):
    ...
    def __init__(self, dev_mode=False):
        super().__init__()
        self._dev_gaze_overlay = None
        self._dev_gaze_timer = None
    ...
    def _start_dev_gaze_overlay(self):
        if not self.dev_mode:
            return
        if not self.gaze_engine or not self.gaze_engine.is_calibrated():
            return

        if self._dev_gaze_overlay is None:
            self._dev_gaze_overlay = GazeDotOverlay(self.stack, radius=16)

        self._dev_gaze_overlay.setGeometry(0, 0, self.stack.width(), self.stack.height())
        self._dev_gaze_overlay.raise_()
        self._dev_gaze_overlay.setVisible(True)

        if self._dev_gaze_timer is None:
            self._dev_gaze_timer = QTimer(self)

        def tick():
            if not self._dev_gaze_overlay or not self._dev_gaze_overlay.isVisible():
                return
            if not self.gaze_engine or not self.gaze_engine.is_calibrated():
                self._dev_gaze_overlay.clear()
                return
            ...
            stack_pos_in_win = self.stack.mapTo(self, QPoint(0, 0))
            ox = int(gx - stack_pos_in_win.x())
            oy = int(gy - stack_pos_in_win.y())

            ox = max(0, min(self._dev_gaze_overlay.width() - 1, ox))
            oy = max(0, min(self._dev_gaze_overlay.height() - 1, oy))

            self._dev_gaze_overlay.set_gaze_pos(ox, oy)

        try:
            self._dev_gaze_timer.timeout.disconnect()
        except Exception:
            pass

        self._dev_gaze_timer.timeout.connect(tick)
        self._dev_gaze_timer.start(30)

    def _stop_dev_gaze_overlay(self):
        if self._dev_gaze_timer is not None:
            self._dev_gaze_timer.stop()
            self._dev_gaze_timer = None
        if self._dev_gaze_overlay is not None:
            self._dev_gaze_overlay.setVisible(False)
            self._dev_gaze_overlay.clear()
