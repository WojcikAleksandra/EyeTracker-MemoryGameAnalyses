...
class RealTimeGazeTrackerWithValidation:
    ...
    - do history dodaje tylko klatki, które przeszły walidację,
    - jeśli bieżąca klatka jest DOBRA:
        -> oblicza predykcję i od razu ją wyświetla (ew. lekko wygładza z poprzednią),
    - jeśli bieżąca klatka jest ZŁA (brak twarzy / złe oczy / mrugnięcie):
        -> jako bieżący punkt spojrzenia pokazuje średnią z kilku ostatnich DOBRYCH predykcji,
    - wyświetla punkty kontrolne i zbiera błędy po kliknięciach,
    - po zakończeniu (klawisz 'q') wypisuje statystyki błędu.
    """
    ...
    def __init__(
        ...
        self.control_errors = []  # lista odległości (w pikselach)
    ...
    def _update_gaze(self, result: dict):
        if self.validator.is_valid_frame(result):
            ...
            if len(self.history) > 0:
                ...
                alpha = 0.7  # waga nowej próbki; 0.7 = 70% nowej, 30% poprzedniej
                hx = alpha * gx + (1.0 - alpha) * prev_x
                hy = alpha * gy + (1.0 - alpha) * prev_y
                self.current_gaze = (int(hx), int(hy))
            else:
                self.current_gaze = (int(gx), int(gy))
